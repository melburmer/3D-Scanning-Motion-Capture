...
struct GaussianCostFunction
{
	...
	bool operator()(const T* const mu, const T* const sigma, T* residual) const
	{
		...
		T sigma_val = *sigma;
		T mu_val = *mu;
		T two_pi_sigma_square = ceres::sqrt(T(2) * T(3.14) * sigma_val * sigma_val);
		T exp_part = ceres::exp(-T(1.) / (T(2.) * sigma_val * sigma_val) * (point.x - mu_val) * (point.x - mu_val));


		residual[0] = point.y - ((T(1.)/two_pi_sigma_square)*exp_part);
