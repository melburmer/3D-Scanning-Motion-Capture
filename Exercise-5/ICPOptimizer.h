...
public:
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        T p_s_current[3], p_s_init[3], p_t_current[3];
        fillVector(m_sourcePoint, p_s_init); // eigen vector to T
        // apply pose increment
        pose_increment.apply(p_s_init, p_s_current); // translation matrix * source points
        fillVector(m_targetPoint, p_t_current);
        
        for (unsigned i : {0, 1, 2})
            residuals[i] = (p_s_current[i] - p_t_current[i]) * T(m_weight) * T(LAMBDA);
...
public:
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        // Important: Ceres automatically squares the cost function.
        PoseIncrement<T> pose_increment(const_cast<T* const>(pose));
        T p_s_current[3], p_s_init[3], p_t_current[3], n_t_current[3];

        // set vectors with fillVector method
        fillVector(m_sourcePoint, p_s_init);
        fillVector(m_targetPoint, p_t_current);
        fillVector(m_targetNormal, n_t_current);


        pose_increment.apply(p_s_init, p_s_current); // apply pose increment
        // calculate residual

        //T difference{p_s_current - p_t_current};
        //residuals[0] = T(LAMBDA) * (n_t_current[0] * difference[0] + n_t_current[1] * difference[1] + n_t_current[2] * difference[2]);
        residuals[0] = T(LAMBDA) * (n_t_current[0] * (p_s_current[0] - p_t_current[0]) + n_t_current[1] * (p_s_current[1] - p_t_current[1]) + n_t_current[2] * (p_s_current[2] - p_t_current[2]));
    ...
    void pruneCorrespondences(const std::vector<Vector3f>& sourceNormals, const std::vector<Vector3f>& targetNormals, std::vector<Match>& matches) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            if (match.idx >= 0) {
                ...
                // cos teta = |(n1 . n2) / |(|n1|.|n2|) ref: https://www.cuemath.com/algebra/angle-between-two-planes/
                double cos_teta{ sourceNormal.dot(targetNormal) / (sourceNormal.norm() * targetNormal.norm()) };

                double teta{std::acos(cos_teta) * (180.0 / 3.141592653589793238463) }; // radian to degree conversion
                //std::cout << "Angle is: " << teta << std::endl;
                //std::cout << "debug";
...
private:
    ...
    Matrix4f estimatePosePointToPlane(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            // Note: that the system has 4n rows where n is the number of points
            // For each point, we expect you to add both a point-to-plane constraint (one row) as described in
            // the paper, and a point - to - point constraint(three rows, one for each coordinate).
